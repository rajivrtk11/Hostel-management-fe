...
export const postAttendance = (attendance) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.post(BASE_URL+`/attendance/`, attendance, config);
    dispatch({
...
export const getAnalysisByDate = (date) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.post(
      BASE_URL+`/attendance/getAnalysis`,
      { date: date },
      config
    );
...
export const deleteAttendanceByDate = (days) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.delete(BASE_URL+`/attendance/${days}`, config);
